<template>
  <div>
    <v-row>
      <!-- Category Menu -->
      <v-col cols="5" class="space px-1 pb-5" xl="2" lg="2" md="3" sm="3">
        
        <v-card class="mx-auto" loading="yellow" shaped raised min-width="250">
          <v-navigation-drawer bottom permanent color="transparent">
            <v-list dense nav >
              <v-expansion-panels
      hover
      >
      ...
      <v-col cols="10">
        <v-row>
          <v-col cols="12" class="space px-1 pb-5" xl="3" lg="4" md="5" sm="6" v-for="(game, index) in gameList" :key="index">
            <v-hover
          v-slot="{ hover }"
          close-delay="200"
        >  
            ...
            :class="{ 'on-hover': hover }"
              max-width="300" dark flat outlined @click="goToGameDetails(game)" height="500">
                ...
                <v-card-text>
                  ...
                  <v-card-title class="white--text text-grey-darken-1 caption mt-n4">{{ game.category.name}}</v-card-title>
                  <v-card-title class="yellow--text mt-n4">{{ game.budget }} Kyats</v-card-title>
                  <v-card-title class="white--text text-grey-darken-1 caption mt-n4">{{ game.platform.name}}</v-card-title>
                  <v-card-actions class="mx-2 mt-n4">
            <v-spacer></v-spacer>
            <router-link class="mx-2 navlink" to="/cart" color="white">
